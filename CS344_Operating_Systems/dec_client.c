...
// Class: CS 344 OS1
// Description: Client that connects to only dec_server via TCP. Reads the contents of
//              the given ciphertext and key files, sends them to the server, receives the
//              decrypted text from the server, and outputs to stdout with appended newline 
//              char.
...
// need a LOT of space for at least one of the plain text messages
...
int main(int argc, char *argv[]) {
  ...
  ciphertext_file = fopen(argv[1], "r");
  // check for errors
  if (ciphertext_file == NULL) {
    error("Error opening ciphertext");
  }
  ...
  // initialize buffers to hold file contents and decrypted result
  ...
  // first, read ciphertext file
  if (fgets(ciphertext, TEXT_BUFFER_SIZE, ciphertext_file) == NULL) {
    fprintf(stderr, "dec_client: fgets CIPHERTEXT failed");
    exit(1);
  }
  // the file must end in a newline, so we strip the last character from ciphertext
  ciphertext[strlen(ciphertext) - 1] = '\0';

  // close ciphertext file
  if (fclose(ciphertext_file) != 0) {
    error("dec_client: Error closing CIPHERTEXT");
  } 

  // next, read key file and check for errors
  if (fgets(keytext, TEXT_BUFFER_SIZE, keytext_file) == NULL) {
    fprintf(stderr, "dec_client: fgets KEYTEXT failed");
    exit(1);
  }
  ...
  if (fclose(keytext_file) != 0) {
    error("dec_client: Error closing KEYTEXT");
  }

  // check that ciphertext contains ONLY allowed characters
  if (!is_allowed(ciphertext, TEXT_BUFFER_SIZE)) {
    // exit with errors
    fprintf(stderr, "dec_client: CIPHERTEXT contains non-allowed characters.\n");
    exit(1);
  }

  // check that keytext contains ONLY allowed characters
  if (!is_allowed(keytext, TEXT_BUFFER_SIZE)) {
    // exit with errors
    fprintf(stderr, "dec_client: KEYTEXT contains non-allowed characters.\n");
    exit(1);
  }

  // check that keytext is at least as long as ciphertext
  if (strlen(ciphertext) > strlen(keytext)) {
    // exit with errors
    fprintf(stderr, "dec_client: KEYTEXT is shorter than CIPHERTEXT!\n");
    exit(1);
  }
  ...
  if ((rv = getaddrinfo(HOST, argv[3], &hints, &servinfo)) != 0) {
    fprintf(stderr, 
      "dec_client: Failed to connect to server on port %s! getaddrinfo: %s\n", 
  ...
  if (p == NULL) {
    fprintf(stderr, "dec_client: Failed to connect to server on port %s!\n", port_num);
    return 2;
  }
  ...
  if (send_msg(socket_fd, buffer) == -1) {
    error("dec_client: ERROR writing AUTHENTICATE to socket");
  }
  ...
  if (strcmp(buffer, AUTHENTICATE) != 0) {
    fprintf(stderr, "dec_client: Could not connect to server on port %s!\n", port_num);
    exit(2);
  }
  ...
  // ciphertext buffer is ready to go, so we will send it as is
  if (send_msg(socket_fd, ciphertext) == -1) {
    error("dec_client: ERROR writing CIPHERTEXT to socket");
  }
  //printf("dec_client: Sent %s\n", ciphertext);
  ...
  if (send_msg(socket_fd, keytext) == -1) {
    error("dec_client: ERROR writing KEYTEXT to socket");
  }
  //printf("dec_client: Sent %s\n", keytext);
  ...
  if (recv_msg(socket_fd, plaintext, store_msg, TEXT_BUFFER_SIZE, MSG_BUFFER_SIZE) == -1) {
    error("dec_client: ERROR reading PLAINTEXT from socket");
  }


  // --------------------------------------------------------------------
  // OUTPUT PLAINTEXT AND CLOSE CONNECTION
  // --------------------------------------------------------------------
  printf("%s\n", plaintext);
