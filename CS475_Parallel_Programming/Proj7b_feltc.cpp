...
// Project 7b Source Code 
// Christopher Felt
// Sources: Project 7b source code example by Mike Bailey
...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			...
			MPI_Recv( tmpSums, MAXSHIFTS, MPI_FLOAT, src, TAG_GATHER, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
			for( int s = 0; s < MAXSHIFTS; s++ )
				BigSums[s] += tmpSums[s];
		}
	}
